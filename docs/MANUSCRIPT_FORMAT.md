...
Status: **approved on 2026-08-22; parsing, reconciliation, outline, guarded initial creation/import, stable-ID path repair, descriptive metadata editing, and sibling reordering are implemented**
...
The metadata editor applies the same replacement boundary to one selected chapter or scene. It previews each changed title, synopsis, point of view, location, story date, status, ordered label list, planning note, word target, or compile-inclusion value at its exact JSON path. Empty optional inputs remove their fields; `includeInCompile: true` remains the omission default. Confirmation freshly rereads the structure, regenerates the same semantic plan, validates and atomically replaces the complete file, and rereads the exact result. It preserves unknown supported-version fields and unaffected order, never edits bound Markdown, awards no daily credit, and replaces the same guarded one-step in-memory Undo rather than building a hidden history.

The first reorder mutation moves one selected item before its previous sibling or after its next sibling. Top-level chapters and loose scenes share their manuscript's `items` order; a chapter scene moves only inside that chapter's `children` order. The preview names the item, neighbor, direction, one-based positions, container, and exact sibling-array JSON path. Confirmation freshly rereads and derives the same swap, preserves the complete raw item objects, validates and atomically replaces only the structure file, and focuses the moved outline row after success.

Cross-container relocation is a separate explicit scene action. It may transfer a complete loose-scene object into a child-scene array, a child scene to the manuscript top level, or a scene between chapters in the same manuscript. The current container is never a destination; a one-file chapter cannot accept children; and filesystem location never implies hierarchy. The preview names both exact arrays and positions. Confirmation freshly derives the identical transfer and atomically replaces only the structure file, so stable identity, source binding, metadata, and unknown supported fields move together while every Markdown file remains at its existing path.
...
Before export, the app stable-reads the structure and every included source. A missing, changed, ambiguous, symbolic-link, oversized, or unreadable source blocks export and appears in a complete report. The writer may resolve that blocker by repairing a unique stable-ID move, explicitly locating a path-only source, creating the expected source with create-new protection, durably excluding the item, or previewing removal of only its structure entry with guarded Undo. Removal never deletes prose. Export never makes a temporary guessed omission.
...
The first output adapters are Markdown and plain text. They write only to a writer-chosen native Save As destination, always use create-new protection, and never modify source files or structure. Markdown compilation emits one selected manuscript title, exact writer-authored chapter titles, no planning-only scene titles, and `* * *` between scene bodies. It removes only a valid leading structured-note frontmatter block and preserves each remaining Markdown body. Plain text follows the same traversal and boundaries, removes familiar Markdown notation while retaining visible words, and leaves unfamiliar constructs visible rather than discarding them. A compact plan previews order, counts, exclusions, blockers, format, and destination rather than duplicating the full novel. Direct-publishing adapters now include a standards-validated reflowable EPUB and a separately constrained, mechanically verified 6 × 9 inch no-bleed print-interior PDF; neither becomes a prose store. DOCX remains deferred as a possible editable handoff. The evidence-backed boundary is documented in `docs/PUBLISHING_EXPORTS.md`.
