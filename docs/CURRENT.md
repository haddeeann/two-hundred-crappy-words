...
- Slice 0.3.6 adds explicit non-negative daily-total corrections with an append-only local audit, completion recalculation, active-day baseline reset, and serialized writes that briefly pause editing to avoid revision races. The expanded history explains local-date and open/recovery behavior in place. Decision D-019 captures the correction semantics. The full suite has 114 passing tests across fourteen files, Svelte/TypeScript checks report zero warnings, the production build passes, and a temporary in-memory populated-state harness verified the history and correction layouts through window-only capture without changing stored user data. Live correction interaction remains for the combined human demonstration.
