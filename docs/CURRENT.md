...
- Slice 0.3.5 now records each day's applicable goal and adds a collapsed-by-default sidebar history with progressive access to older recorded dates. Its current/best rhythm calculation crosses local calendar boundaries, does not prematurely break a rhythm that reached yesterday, and turns a later gap into a fresh-start message without erasing the best. Decision D-018 preserves those rules. The full frontend suite has 106 passing tests across thirteen files, Svelte/TypeScript checks report zero warnings, the production build passes, and window-only capture verifies both disclosure states at the native window size. Populated-history interaction remains for the combined human demonstration.
- Slice 0.3.6 adds explicit non-negative daily-total corrections with an append-only local audit, completion recalculation, active-day baseline reset, and serialized writes that briefly pause editing to avoid revision races. The expanded history explains local-date and open/recovery behavior in place. Completed days also retain the goal actually reached, avoiding contradictory history after a later target change. Decision D-019 captures the correction semantics. An integration lifecycle test now covers active editing through completion, a later goal change, restart restoration, correction, and earning the corrected remainder; equal ledger revisions can no longer replace existing data. The full suite has 119 passing tests across fifteen files, Svelte/TypeScript checks report zero warnings, the production build and Rust formatting/checks pass, and the production dependency audit reports zero vulnerabilities. A temporary in-memory populated-state window capture passed without changing stored user data. Live interaction remains for the combined human demonstration in `docs/MANUAL_QA.md`.
...
No implementation blocker. The automated milestone work can continue; final roadmap credit awaits one combined restart, goal, populated-history, and correction desktop demonstration using disposable text.
