...
The final blocked-source package added one included scene whose expected Markdown was absent. The compile report exposed **Locate source…**, **Create source…**, **Exclude from compile…**, **Remove scene…**, and **Edit all details…** together, while reserving stable-ID repair for the uniquely moved state it already supports. Locate selected an existing path-only Markdown file, previewed only `$.manuscripts[0].items[0].children[3].source.path`, left that file byte-stable, returned automatically to a ready compile, and shared Undo restored the exact missing binding. Create previewed the exact expected path, created a zero-byte file without overwriting, returned to a ready compile, and left structure and other sources unchanged.

Remove previewed the complete scene entry at `$.manuscripts[0].items[0].children[3]`, named its UUID and expected source, and stated that no Markdown file would be deleted. Confirmation removed only the structure item, retained the unrelated recovery source byte-for-byte, and shared Undo restored the missing scene exactly. Exclude opened the existing metadata preview already unchecked, changed only `includeInCompile`, made the compile ready with the omission reported, and its guarded Undo restored inclusion. Every action used a keyboard-contained, cancel-first surface and returned to a freshly planned compile or the exact invoking outline state. Today stayed `0 / 200`. QA caught a first removal preview that named only the nested `.source` value; that untouched attempt was canceled, the exact whole-item path was corrected, and the rebuilt package passed. Cleanup restored the original three-scene structure and all three source SHA-256 hashes exactly.
